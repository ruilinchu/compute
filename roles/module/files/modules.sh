...
if [ -z "${USER_IS_ROOT:-}" ]; then
  ...
  if [ -z "${MODULEPATH_ROOT:-}" ]; then
    ...
    MODULEPATH_INIT="/opt/apps/lmod/lmod/init/.modulespath"
    if [ -f ${MODULEPATH_INIT} ]; then
       for str in $(cat ${MODULEPATH_INIT} | sed 's/#.*$//'); do   # Allow end-of-line comments.
          for dir in $str; do
              if [ -d $dir ]; then
                  export MODULEPATH=$(/opt/apps/lmod/lmod/libexec/addto --append MODULEPATH $dir)
              fi
	  done
       done
    else
       export MODULEPATH=$(/opt/apps/lmod/lmod/libexec/addto --append MODULEPATH $MODULEPATH_ROOT/$LMOD_sys $MODULEPATH_ROOT/Core)
